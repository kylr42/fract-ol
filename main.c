...
{
	if (ft_strncmp("Julia", arg, 10) == 0 ||
			ft_strncmp("julia", arg, 10) == 0)
		all->settings.fractol = 0;
	else if (ft_strncmp("Mandelbrot", arg, 25) == 0 ||
			 ft_strncmp("Mandelbrot", arg, 25) == 0)
...
static	void	ft_init_window(t_all *all)
{
	...
	all->mlx.img = mlx_new_image(all->mlx.mlx, 1920, 1080);
